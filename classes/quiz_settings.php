...
/**
 * Per-quiz opt-in store for the AI tutor.
 *
...
/**
 * Reads and writes the per-quiz (per course-module) opt-in flag in {local_aitutor_quiz}.
 *
...
class quiz_settings {
    ...
    public static function is_enabled(int $cmid): bool {
        ...
        return (bool) $DB->get_field('local_aitutor_quiz', 'enabled', ['cmid' => $cmid]);
    }
    ...
    public static function set_enabled(int $cmid, bool $enabled): void {
        ...
        $existing = $DB->get_record('local_aitutor_quiz', ['cmid' => $cmid]);
        if ($existing) {
            $existing->enabled = $enabled ? 1 : 0;
            $existing->timemodified = time();
            $DB->update_record('local_aitutor_quiz', $existing);
        } else {
            $DB->insert_record('local_aitutor_quiz', (object) [
                'cmid' => $cmid,
                'enabled' => $enabled ? 1 : 0,
                'timemodified' => time(),
            ]);
    ...
    public static function delete_for_cmid(int $cmid): void {
        ...
        $DB->delete_records('local_aitutor_quiz', ['cmid' => $cmid]);
    }
}
